...
std::set<Estado*> MoverX(Estado *x,char simbolo);
std::set<Estado*> Mover(std::set<Estado*> x,char simbolo);
std::set<Estado*> cerraduraEpsilon(std::set<Estado*> x);
std::set<Estado*> cerraduraEpsilonX(Estado *x);
...
Automata *parseThompson(std::string exp);
std::string edoSetToEdoId(std::set<Estado*> nuevo);
int subconjuntos(Automata *AFN, Automata *AFD);
...
int main(int argc, char *argv[]){
    ...
    subconjuntos(f,n);
    imprimirAutomata(n);
    std::cout << "Inicial: " << n->getInicial()->getId() << std::endl;
	std::cout << "Final: " << n->getFinal()->getId() << std::endl;
...
std::set<Estado*> cerraduraEpsilonX(Estado *x){
    std::list<Transicion*> transiciones = x->getTransiciones();
    std::set<Estado*> alcanzables;
    std::list<Transicion*>::iterator it;
	std::set<Estado*> recursivo;
	std::set<Estado*>::iterator itr;
    alcanzables.insert(x);
    for (it = transiciones.begin(); it != transiciones.end(); it++){
        if((*it)->getSimbolo() == '\0'){
			recursivo = cerraduraEpsilonX((*it)->getDestino()
			for(itr = recursivo.begin() ; itr != recursivo.end(); itr++){
				alcanzables.insert(*itr);
			}
            //alcanzables.insert(cerraduraEpsilonX((*it)->getDestino()));
            alcanzables.insert((*it)->getDestino());
        }
    }
    return alcanzables;
}
...
std::set<Estado*> cerraduraEpsilon(std::set<Estado*> x){
    ...
    std::set<Estado*> alcanzables;
    std::list<Transicion*>::iterator it;
    std::set<Estado*>::iterator eit;
	std::set<Estado*> recursivo;
	std::set<Estado*>::iterator itr;
    for(eit = x.begin(); eit != x.end(); eit++){
        transiciones = (*eit)->getTransiciones();
        alcanzables.insert(*eit);
        for (it = transiciones.begin(); it != transiciones.end(); it++){
            if((*it)->getSimbolo() == '\0'){
				recursivo = cerraduraEpsilonX((*it)->getDestino()
				for(itr = recursivo.begin() ; itr != recursivo.end(); itr++){
...
int subconjuntos(Automata *AFN, Automata *AFD){
    ...
    std::string id;
    std::set<Estado*> temp = cerraduraEpsilonX(AFN->getInicial);
    Estado *nuevo = new Estado;
    id = edoSetToEdoId(temp);
    estados.insert(id);
    nuevo->setId(id);
    AFN->
}
